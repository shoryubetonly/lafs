...
<section class="section">
  <div class="form" style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
    ...
    <div class="actions">
      <a class="btn btn-secondary" href="">↩ กลับหน้าเว็บ</a>
      <?php if (($_SESSION['admin']['role'] ?? '') === 'root'): ?>
        <a class="btn" href="admin_users.php">ผู้ดูแลระบบ</a>
        <a class="btn" href="admin_user_new.php">+ เพิ่มแอดมิน</a>
      <?php else: ?>
        <a class="btn" href="admin_user_edit.php?id=<?= (int)($_SESSION['admin']['id'] ?? 0) ?>">โปรไฟล์ของฉัน</a>
      <?php endif; ?>
      <!-- logout -->
      <form action="admin_logout.php" method="post" style="display:inline; margin-left:6px;">
        ...
        <article class="card">
          ...
          <div class="body">
            ...
            <div class="actions" style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap;">
              <a class="btn btn-secondary" href="view.php?id=<?=(int)$it['id']?>">ดูหน้าโพสต์</a>

              <?php if ($it['status'] === 'open'): ?>
                <form method="post" action="admin_posts.php" onsubmit="return confirm('ยืนยันปิดเคสโพสต์ #<?= (int)$it['id']?> ?');" class="inline-form">
                  <input type="hidden" name="csrf" value="<?=esc(csrf_token())?>">
              ...
              <?php else: ?>
                <form method="post" action="admin_posts.php" onsubmit="return confirm('ยืนยันเปิดเคสโพสต์ #<?= (int)$it['id']?> ใหม่ ?');" class="inline-form">
                  <input type="hidden" name="csrf" value="<?=esc(csrf_token())?>">
                  <input type="hidden" name="id" value="<?= (int)$it['id'] ?>">
              ...
              <form method="post" action="admin_posts.php" onsubmit="return confirm('ลบโพสต์นี้ถาวร? การลบไม่สามารถย้อนกลับได้');" class="inline-form">
                <input type="hidden" name="csrf" value="<?=esc(csrf_token())?>">
                <input type="hidden" name="id" value="<?= (int)$it['id'] ?>">
